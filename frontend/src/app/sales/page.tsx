...
export default function SalesPage() {
  ...
  const handleSave = async () => {
    if (!currentSale || !currentSale.clientId || !currentSale.numero_facture || (currentSale.items?.length || 0) === 0) {
      alert('Veuillez remplir tous les champs obligatoires.');
      return;
    }
    await addSale(currentSale as Omit<Sale, 'id' | 'status'>);
    setView('list');
    setCurrentSale(null);
  };
        ...
        {view === 'list' && (
          ...
          >
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-[var(--sidebar)] p-4 md:p-6 rounded-3xl border border-[var(--card-border)] shadow-premium gap-4">
              <div>
                <h2 className="text-xl md:text-2xl font-black uppercase tracking-tight">Ventes & Facturation</h2>
                <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">Historique et gestion des factures</p>
              </div>
              <button 
            ...
            <div className="glass-card overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[800px]">
                  <thead>
                    <tr className="bg-slate-50 dark:bg-slate-800/50 text-slate-400 text-[10px] uppercase font-black tracking-widest border-b border-[var(--card-border)]">
                  ...
                  <tbody className="divide-y divide-[var(--card-border)]">
                    {sales.map((sale) => (
                      <tr key={sale.id} className="hover:bg-primary-50/30 dark:hover:bg-primary-900/10 transition-colors group">
                        <td className="px-4 md:px-8 py-5">
                          <span className="font-black text-sm font-mono text-primary-600">{sale.numero_facture}</span>
                        </td>
                        <td className="px-4 md:px-8 py-5">
                          <div className="font-bold text-sm truncate max-w-[150px]">{clients.find(c => c.id.toString() === sale.clientId.toString())?.nom || 'Client Inconnu'}</div>
                        </td>
                        <td className="px-4 md:px-8 py-5 text-slate-400 text-[10px] md:text-xs font-bold uppercase">{formatDate(sale.date_facture)}</td>
                        <td className="px-4 md:px-8 py-5">
                          <span className={cn(
                        ...
                        <td className="px-4 md:px-8 py-5">
                          <div className="flex justify-center gap-2">
                            <button 
                              ...
                              className="p-2.5 bg-indigo-500 hover:bg-indigo-600 text-white rounded-xl transition-all shadow-lg shadow-indigo-500/20 active:scale-95"
                              title="Imprimer"
                            >
                              <Printer size={16} />
                            </button>
                            {sale.status === 'DRAFT' && (
                              <button 
                                ...
                                className="p-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl transition-all shadow-lg shadow-emerald-500/20 active:scale-95"
                                title="Valider"
                              >
                                <CheckCircle size={16} />
                              </button>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 md:gap-8">
              <div className="xl:col-span-4 space-y-4 md:space-y-6">
                <div className="glass-card p-6 md:p-8 space-y-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Type de Commerce</label>
                    <div className="grid grid-cols-2 bg-slate-100 dark:bg-slate-800/50 p-1.5 rounded-2xl">
                      <button 
                        onClick={() => setCurrentSale({...currentSale, type_vente: 'materiel', items: []})}
                        className={cn(
                          "py-2.5 rounded-xl text-[10px] font-black transition-all",
                          currentSale.type_vente === 'materiel' ? "bg-primary-600 text-white shadow-lg" : "text-slate-500"
                        )}
                      >
                        MATÉRIEL
                      </button>
                      <button 
                        ...
                        className={cn(
                          "py-2.5 rounded-xl text-[10px] font-black transition-all",
                          currentSale.type_vente === 'cafe' ? "bg-primary-600 text-white shadow-lg" : "text-slate-500"
                        )}
                      >
                        CAFÉ
                      </button>
                  ...
                  <div className="space-y-2">
                    ...
                    <select 
                      ...
                      onChange={(e) => setCurrentSale({...currentSale, clientId: e.target.value})}
                    >
              ...
              <div className="xl:col-span-8 flex flex-col gap-6">
                <div className="glass-card flex-1 flex flex-col min-h-[400px]">
                  <div className="p-4 md:p-6 border-b border-[var(--card-border)] flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl bg-primary-600 flex items-center justify-center text-white">
                        <Plus size={18} />
                      </div>
                  ...
                  <div className="flex-1 overflow-x-auto overflow-y-hidden">
                    <table className="w-full min-w-[600px]">
                      <thead>
                        <tr className="text-[10px] uppercase font-black tracking-widest text-slate-400 border-b border-[var(--card-border)]">
                      ...
                      <tbody className="divide-y divide-[var(--card-border)]">
                        {currentSale.items?.map((item, index) => (
                          <tr key={index} className="group hover:bg-slate-50 dark:hover:bg-slate-800/30">
                            ...
                            <td className="px-6 py-4 text-center">
                              <input 
                                type="number" 
                                className="w-16 bg-slate-100 dark:bg-slate-800 border-none rounded-lg text-center font-black py-2 text-xs md:text-sm outline-none"
                                value={item.quantite}
                            ...
                            <td className="px-6 py-4 text-center">
                              <button onClick={() => removeItem(index)} className="text-slate-400 hover:text-rose-500 p-2">
                                <Trash2 size={16} />
                              </button>
                            </td>
                          </tr>
