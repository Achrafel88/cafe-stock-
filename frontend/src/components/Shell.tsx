    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="p-6 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2 overflow-hidden">
          <div className="w-8 h-8 rounded-lg bg-primary-600 flex items-center justify-center text-white font-bold">C</div>
        ...
        <button 
          ...
          className="p-2 hover:bg-primary-50 dark:hover:bg-slate-800 rounded-xl transition-colors hidden md:block"
        >
          {isSidebarOpen ? <X size={20} /> : <MenuIcon size={20} />}
        </button>
        <button 
          onClick={() => setIsMobileMenuOpen(false)} 
          className="p-2 hover:bg-primary-50 dark:hover:bg-slate-800 rounded-xl transition-colors md:hidden block"
        >
          <X size={20} />
        </button>
      </div>
          ...
          return (
            ...
            >
              <div className={cn(
                ...
                !isActive && "bg-slate-50 dark:bg-slate-900/50 group-hover:border-primary-200 dark:group-hover:border-primary-800",
        ...
        <div className="flex items-center justify-between">
          {mounted && (
            <button 
              ...
              className="p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-slate-600 dark:text-slate-400 hover:text-primary-600 transition-all flex-1 flex justify-center items-center group"
            >
              {theme === 'dark' ? <Sun size={20} /> : <Moon size={20} />}
              {(isSidebarOpen || isMobileMenuOpen) && <span className="ml-3 text-sm font-bold">Thème</span>}
            </button>
          )}
        </div>
      </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className="h-16 md:h-20 border-b border-[var(--card-border)] bg-[var(--background)]/80 backdrop-blur-md flex items-center px-4 md:px-8 justify-between print-hidden z-30 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-lg md:text-2xl font-black uppercase tracking-tight text-slate-900 dark:text-white truncate max-w-[150px] md:max-w-none">
          ...
          <div className="flex items-center gap-3 md:gap-6">
            <div className="hidden sm:flex flex-col items-end">
              ...
              <p className="text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-tighter">BENI MELLAL • MAROC</p>
            </div>
            <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-gradient-to-tr from-primary-600 to-indigo-500 p-[2px] shrink-0 shadow-lg shadow-primary-600/20">
              <div className="w-full h-full rounded-[9px] md:rounded-[14px] bg-white dark:bg-[#0f172a] flex items-center justify-center font-black text-primary-600 text-sm md:text-base">
                BH
              </div>
            </div>
          </div>
