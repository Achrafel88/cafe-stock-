...
export default function AnnualReportView({ year, products, movements, type }: AnnualReportProps) {
  ...
  return (
    <div className="bg-white text-black min-h-screen p-4 md:p-8 font-sans max-w-[900px] mx-auto print:m-0 print:p-8">
      {/* Header */}
      <div className="flex flex-col md:flex-row justify-between items-start mb-6 md:mb-10 border-b-2 border-primary-900 pb-4 gap-4">
        ...
        <div className="text-left md:text-right flex flex-col items-start md:items-end">
          <div className="bg-black bg-primary-800 text-foreground text-primary-100 px-4 py-2 font-black text-lg md:text-xl mb-4 uppercase">
            RAPPORT ANNUEL {year}
          </div>
      ...
      <div className="mb-6">
        <h2 className="text-center text-lg md:text-xl font-black uppercase border-2 border-primary-900 py-2 bg-primary-50 px-2">
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse border-2 border-primary-900 mb-8 min-w-[600px]">
          <thead>
            <tr className="bg-gray-200 text-[9px] md:text-[10px]">
              <th className="border-2 border-primary-900 px-3 py-2 text-left uppercase font-black">Désignation Produit</th>
              <th className="border-2 border-primary-900 px-2 py-2 text-center uppercase font-black w-16">Unité</th>
              <th className="border-2 border-primary-900 px-2 py-2 text-right uppercase font-black w-24">Stock Initial</th>
              <th className="border-2 border-primary-900 px-2 py-2 text-right uppercase font-black w-24 bg-primary-50">Entrées</th>
              <th className="border-2 border-primary-900 px-2 py-2 text-right uppercase font-black w-24 bg-primary-50">Sorties</th>
              <th className="border-2 border-primary-900 px-2 py-2 text-right uppercase font-black w-24">Stock Actuel</th>
            </tr>
          </thead>
          <tbody>
            {reportData.map((item, i) => (
              <tr key={i} className="border-2 border-primary-900">
                <td className="border-2 border-primary-900 px-3 py-2 text-xs md:text-sm font-bold uppercase">{item.designation}</td>
                <td className="border-2 border-primary-900 px-2 py-2 text-center text-[9px] md:text-[10px] font-black uppercase">{item.unite}</td>
                <td className="border-2 border-primary-900 px-2 py-2 text-right text-xs md:text-sm font-mono">{item.stockInitial}</td>
                <td className="border-2 border-primary-900 px-2 py-2 text-right text-xs md:text-sm font-black font-mono text-primary-600 bg-primary-50/30">{item.totalIn}</td>
                <td className="border-2 border-primary-900 px-2 py-2 text-right text-xs md:text-sm font-black font-mono text-primary-600 bg-primary-50/30">{item.totalOut}</td>
                <td className="border-2 border-primary-900 px-2 py-2 text-right text-xs md:text-sm font-black font-mono bg-gray-50">{item.currentStock}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="mt-6 md:mt-10 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 text-xs md:text-sm">
        <div className="border border-primary-900 p-4">
          ...
          <div className="space-y-1 font-bold">
            ...
            <p>Somme des entrées: <span className="float-right text-primary-600">{reportData.reduce((acc, d) => acc + d.totalIn, 0)}</span></p>
            <p>Somme des sorties: <span className="float-right text-primary-600">{reportData.reduce((acc, d) => acc + d.totalOut, 0)}</span></p>
          </div>
        </div>
        <div className="text-center flex flex-col justify-center border border-primary-900 p-4 min-h-[100px]">
          <p className="text-[9px] md:text-[10px] font-black uppercase underline mb-6">Signature & Cachet</p>
        </div>
      </div>

      {/* Business Footer */}
      <div className="mt-auto pt-10 border-t border-gray-300 text-[10px] text-primary-500 text-center leading-relaxed">
